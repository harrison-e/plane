...
void main()
{
  ...
  // // Water is below 2 (sparse in the desert)
  // if (FragPos.y < 2.f) {
  //   diffuseColor = vec3(0.0, 0.4, 0.6);
  // }
  // Various sand and rock colors at different elevations
  if (FragPos.y < 15.f){
    // orange sand
    diffuseColor = vec3(0.8, 0.6, 0.3);
  }
  else if (FragPos.y < 30.f) {
    // brighter rock
    diffuseColor = vec3(0.82, 0.62, 0.32);
  }
  else if (FragPos.y < 35.f) {
    // redder rock
    diffuseColor = vec3(0.89, 0.62, 0.32);
  }
  else if (FragPos.y < 50.f) {
    // brighter rock
    diffuseColor = vec3(0.82, 0.62, 0.32);
  }
  else if (FragPos.y < 60.f) {
    // even brighter rock
    diffuseColor = vec3(0.82, 0.62, 0.32);
  }
  else if (FragPos.y < 65.f) {
    // darker rock
    diffuseColor = vec3(0.72, 0.52, 0.22);
  }
  else if (FragPos.y < 80.f) {
    // redder rock
    diffuseColor = vec3(0.89, 0.62, 0.32);
  }
  else {
    // plateau
    diffuseColor = vec3(0.91, 0.64, 0.32);
  }
  ...
  // vec3 detailColor    = texture(u_DetailMap,  v_texCoord).rgb;
    ...
    if(gl_FrontFacing){
        FragColor = vec4(vec3(diffuseColor * Lighting),1.0);
    }else{
